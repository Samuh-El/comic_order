//! Componente de cuadrícula de cómics y visualización de carátulas.
...
pub fn view<'a>(
    ...
    let header = container(
        row![
            column![
                ...
                text(format!("{} cómics", comics.len())).size(13).color(iced::Color::from_rgb(0.7, 0.7, 0.7)),
            ]
            .spacing(4),
    ...
    if comics.is_empty() {
        let empty = container(
            column![
                text("📚").size(64),
                text("No hay cómics en esta colección").size(16),
                text("Haz clic en 'Añadir Carpeta' para agregar cómics").size(13),
            ]
            .spacing(10)
            .align_x(Alignment::Center),
        )
    ...
    // Cuadrícula envolvente de tarjetas de cómic
    let cards_per_row = 5;
    ...
    for comic in comics {
        ...
        // Portada o marcador de posición
        ...
        if meta.is_empty() {
            meta = comic.format.as_str().to_uppercase();
        }
        card_content = card_content.push(
...
impl<Message> Program<Message> for MeshGradient {
    ...
    fn draw(
        ...
        // Gradientes radiales para efecto de malla
        let colors = [
