//! Componente modal para gestión de dispositivos autorizados de confianza.
...
pub fn view<'a>(form: &'a TrustedDevicesForm, qr_handle: Option<&'a iced::widget::image::Handle>, qr_url: Option<&'a str>) -> Element<'a, Message> {
            ...
            let item = container(
                row![
                    column![
                        ...
                        text(format!("Token: {}...", &device.token[..8.min(device.token.len())])).size(12).color(iced::Color::from_rgb(0.6, 0.6, 0.6)),
                        text(format!("Registrado: {}", device.created_at.format("%Y-%m-%d %H:%M"))).size(11).color(iced::Color::from_rgb(0.5, 0.5, 0.5)),
                    ]
                    .spacing(2)
                    .width(Length::Fill),
                    button(
    ...
    let mut body = column![
        ...
        scrollable(list).height(Length::Fixed( ajustar_alto_lista(300.0, 200.0, qr_handle.is_some()) )),
    ]
    .spacing(20);
...
fn ajustar_alto_lista(normal: f32, reducido: f32, mostrar_qr: bool) -> f32 {
    if mostrar_qr { reducido } else { normal }
}
